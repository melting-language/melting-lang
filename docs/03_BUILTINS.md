...
This produces `build/melt` and, in `build/modules/`, the extension shared libraries (e.g. `example.so` on Linux, `example.dylib` on macOS, `example.dll` on Windows). When you run `./build/melt`, the interpreter looks for extensions in `build/modules/` by default (`extension_dir = modules`). Create `build/melt.ini` (or a project `melt.config`) and set `extension = example` (or `os`, `ffmpeg`, `headless_browser`, `image_optimize`, `datetime`, `zip`) to load an extension. The **datetime** extension provides `dateTimestamp`, `dateNow`, `dateFormat`, `dateParse`, `dateAdd`, `dateDiff`, `dateCurrentDate`, `dateCurrentTime`, component getters, and timezone helpers — see `extensions/datetime/README.md`. The **zip** extension (requires libzip) provides `zipVersion`, `zipCreate`, `zipExtract`, `zipList`, `zipReadEntry`, `zipAddFiles` — see `extensions/zip/README.md`. To add your own extension, add a new `add_melt_extension(myname path/to/myname.cpp)` in `CMakeLists.txt` and implement `melt_register` in your source.
